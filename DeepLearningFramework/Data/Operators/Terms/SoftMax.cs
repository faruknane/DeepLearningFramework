    ...
    public class SoftMax : Term
    {
        ...
        public override unsafe void CalculateDerivate(MMDerivative s)
        {
            ...
            for (int i1 = 0; i1 < s.D1; i1++)
                ...
                {
                    Vectorization.ElementWiseMultiplyAVX(sm.Array, s.Derivatives + i1 * s.D2 * s.D3 * s.D4 + i2 * s.D3 * s.D4, combined.Derivatives + i1 * s.D2 * s.D3 * s.D4 + i2 * s.D3 * s.D4, sm.D1 * sm.D2);
                    //combined[i1, i2, x3, x2] += s[i1, i2, x3, x2] * sm[x3, x2];
                }
            ...
            //combined[i1, i2, x3, x2] += s[i1, i2, x3, x2] * (sm[x3, x2]) * (1 - sm[x3, x2]);
